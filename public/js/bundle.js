...
var displayMap = function displayMap(locations) {
  ...
  var createMarker = function createMarker(location, i) {
    var latlng = [location.coordinates[1], location.coordinates[0]];
    new L.marker(latlng).addTo(map).bindPopup(L.popup({
      autoClose: false,
      closeButon: false
    }).setContent("<b>Day ".concat(location.day, " : ").concat(location.description, "</b>"))).openPopup();
    bounds.extend(latlng);
  };

  locations.forEach(function (el, i) {
    createMarker(el, i);
  });
      ...
      while (1) {
        switch (_context3.prev = _context3.next) {
          ...
          case 3:
            ...
            if (res.data.status === 'success') {
              (0, _alerts.showAlert)('success', 'Registered succesfully');
              window.setTimeout(function () {
                location.assign('/');
              }, 1000);
            }
          ...
          case 7:
            ...
            (0, _alerts.showAlert)('error', 'Problem occured during registration. Try again!');
...
var stripe = Stripe('pk_test_51LQcJxL5qFqdnk7Ew1qKyEYWvGGYp2XMIsDt1YoGgajjHlilHfUbTG7FhjkbZZrFAtQShGodjqPD0vnGQZB7nssD00PfUckDvZ');
...
var deleteBtn = document.querySelector('.btn--delete'); //values
...
if (registerForm) {
  registerForm.addEventListener('submit', function (e) {
    e.preventDefault();
    (0, _login.register)(nickname.value, email.value, password.value, passwordConfirm.value);
  });
}
